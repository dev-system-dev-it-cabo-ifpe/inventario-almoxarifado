    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.assets.update", [$asset->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="balancete">{{ trans('cruds.asset.fields.balancete') }}</label>
                <input class="form-control {{ $errors->has('balancete') ? 'is-invalid' : '' }}" type="text" balancete="balancete" id="balancete" value="{{ old('balancete', $asset->balancete) }}" required>
                @if($errors->has('balancete'))
                    <div class="invalid-feedback">
                        {{ $errors->first('balancete') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.asset.fields.balancete_helper') }}</span>
            </div>

            <div class="form-group">
                <label class="required" for="codigo">{{ trans('cruds.asset.fields.codigo') }}</label>
                <input class="form-control {{ $errors->has('codigo') ? 'is-invalid' : '' }}" type="text" codigo="codigo" id="codigo" value="{{ old('codigo', $asset->codigo) }}" required>
                @if($errors->has('codigo'))
                    <div class="invalid-feedback">
                        {{ $errors->first('codigo') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.asset.fields.codigo_helper') }}</span>
            </div>
